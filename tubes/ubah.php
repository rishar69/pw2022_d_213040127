...
if( isset($_POST["submit"])){
    if (ubah($_POST) > 0 ){
        echo "<script>
                alert('Product Changed Succsessfuly!')       
                document.location.href = 'shop.php';
            </script>";
    }else{
      echo "<script>
                alert('Data failed to be changed!')       
...
<nav class="navbar navbar-expand-lg warna-navbar fixed-top">
    ...
    <div class="container" style="background-color: #f7e4f0 ";>
      <h1>Edit Product</h1>
        <div class="row mt-3">
            <div class="col-8">
            <form action="" method="post" enctype="multipart/form-data">
              <input type="hidden" name="id"  value="<?=$produk[0]["id_barang"];?>">
              <input type="hidden" name="gambarLama"  value="<?=$produk[0]["gambar"];?>">

            <div class="mb-3">
                <label for="nama" class="form-label">Name</label>
                <input type="text" class="form-control" id="nama" name="nama" value="<?=$produk[0]["nama"];?>" required>
                </div>
            <div class="mb-3">
                <label for="dekripsi" class="form-label">Description</label>
                <textarea type="text" class="form-control" id="dekripsi" name="dekripsi" style="height:120px"><?=$produk[0]["dekripsi"];?> </textarea>
         
            </div>
            <div class="mb-3">
                <label for="harga" class="form-label">Price</label>
                <input type="number" class="form-control" id="harga" value="<?=$produk[0]["harga"];?>" name="harga">
            </div>         
            <div class="mb-3">
            ...
            <button style="background-color: pink ;" type="submit" name="submit" class="btn ">Edit Product</button>
        </form>
            </div>
        </div>
